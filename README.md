...
`echo.yaml` is written by three different kinds of contributors, and they remain distinct:

| layer | written by | examples |
|---|---|---|
| `_generated` | tools, automation | file size, fork source from API |
| `meta`, `origin`, `resonance` | the creator | intent, influence, lineage |
| `audience` | those who received the work | experience, entry point, personal note |

What a creator meant and what an audience felt are different things. ECHO keeps them separate.
...
An echo is what remains after something has been heard. It travels. It fades into other echoes. It does not ask permission.
...
`echo_version`以外のすべてのフィールドは省略可能です。最小限の`echo.yaml`はこれだけで成立します：
...
エコーとは、何かが聴かれたあとに残るものです。それは移動し、他のエコーに溶け込みます。許可を求めません。
